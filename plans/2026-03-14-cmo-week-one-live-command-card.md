# ACE Week-One Live Command Card
...
do not depend on jumping across long docs while the launch window is live.
...
## Open These Before Any Live Motion

Keep these surfaces open before the live window starts:

1. `plans/2026-03-16-cmo-launch-operations-log.md`
2. `plans/2026-03-13-cmo-week-one-live-capture-sheet.md`
3. `plans/2026-03-13-cmo-week-one-response-bank.md`
4. `plans/2026-03-13-cmo-week-one-publishing-surface.md`
5. `plans/2026-03-13-cmo-week-one-profile-surface.md` on Monday only if the
   anchor thread still needs to be pinned after QA
...
- do not leave a design-partner row active without a real `Next step due by`
  date
...
- publish between `11:00` and `11:30` ET
...
6. move any qualified builder into DM the same day with the governed routing
   path
7. capture one reusable artifact: a strong thesis restatement, a screenshot-
   worthy metric jump, or a repeated pain phrase
8. by end of day, update the `Execution Log`, `Immediate Publish QA Log`,
   `Engagement Follow-Up Log`, pipeline, friction log, and `Content Reuse Queue`

Done means:

- live thread URL is logged
- QA is complete
- pin status is explicit
...
- run the distribution pass between `12:00` and `13:00` ET
- DM-routing cleanup by `16:00` ET
- day-end logging by `17:30` ET
...
1. publish the governed proof post with `w1_proof_demo`
2. confirm `docs/assets/ace-proof-demo/still.svg` is attached and rendered in
   the raw `Immediate Publish QA Scratchpad`
3. reply publicly to technical skepticism that can strengthen inspectability
4. DM only builders who ask how ACE fits a real workflow
5. route any builder with a clear repeated failure into
   `issue_w1_failure_pattern` when public intake is viable
6. capture one reusable proof artifact: a technical objection, save or bookmark
   signal, or public workflow report
7. update the `Execution Log`, `Immediate Publish QA Log`,
   `Engagement Follow-Up Log`, `Workflow Intake Log`, pipeline, friction log,
   and `Content Reuse Queue`

Done means:

- live proof URL is logged
- asset QA is complete
...
1. publish the governed adoption post with `w1_adoption_example`
2. complete the raw `Immediate Publish QA Scratchpad` within `5` minutes
3. ask strong responders for one concrete repeated failure pattern
4. default to `dm_w1_example_doc` unless the builder explicitly needs MCP or
   API detail
5. route the strongest public-fit builder into `issue_w1_failure_pattern`
6. capture one adoption artifact: a setup objection, setup-start signal, or
...
Before closing any launch day:

1. move raw notes into the canonical launch log
2. confirm every qualified interaction has a logged next step or an explicit
